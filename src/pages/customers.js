...
import { getAllOffers, updateData } from '../utils/custFetch';
...
const Page = ({data}) => {
  ...
  useLayoutEffect(() => {
    ...
    queryParams.set('page', `${rowsPerPage}`);
  ...
  const filterOfferById = useCallback(() => {
    ...
    if (isOpenModal.idOffers) {
      offer = data.find(ele => ele.id === isOpenModal.idOffers);
    }
    return offer;
  },
    [isOpenModal.idOffers]
  );
